  ...
  immer<ISpringState & ISpringActions>((set) => ({
    ...
    calculate: () => {
      set((state) => {
        ...
        state.reactVertices.map((vertex, index) => {
          if (verticesNames.includes(vertex.name)) {
            state.errors.vertices.push({
              ...
              message: `El nudo  ${index + 1} llamado ${vertex.name
                } ya existe!`,
              typeError: 'userInput',
              errorCode: 1,
            })
        ...
        state.reactEdges.map((edge, index) => {
          if (edgesNames.includes(edge.name)) {
            state.errors.edges.push({
              ...
              message: `El resorte  ${index + 1} llamado ${edge.name
                } ya existe!`,
              typeError: 'userInput',
              errorCode: 4,
            })
        ...
        const { global: fGlobal,
          restricted: fRestricted,
          unrestricted: fUnrestricted, } = spring.buildForces()
